...
void _loadExistingRecord(Map<String, dynamic> record) async {
  ...
  final chemicalList = (response['chemical_products'] as List<dynamic>?)
    ?.map((e) {
  ...
  final organicList = (response['organic_treatments'] as List<dynamic>?)
          ?.map((e) => e.toString())
          .toList() ??
      [];
 ...
 for (var item in chemicalList) {
  ...
  final approx_cost = await _withNativeTranslation(item['approx_cost'] ?? '');
  ...
  Future<void> _sendImageForPrediction(File imageFile) async {
    ...
    if (response.statusCode != 200) {
      ...
      _translatedText = await _translateToNativeLanguage("The diagnosis failed.", targetLangCode);
    ...
    final chemicalList = (jsonData['chemical_products'] as List<dynamic>? ?? []).cast<Map<String, dynamic>>();
    final organicList = (jsonData['organic_treatments'] as List<dynamic>? ?? []).cast<String>();
...
Future<void> _speakFullResult(String diagnosis, String symptoms, List<Map<String, String>> chemicalProducts, List<String> organicTreatments, String targetLangCode) async {
  ...
  _translatedText = await _translateToNativeLanguage(fullText, targetLangCode);
...
void faileddiagnosis() async {
  ...
  String translated = await _translateToNativeLanguage(failedMsg, targetLangCode);
  setState(() {
...
Widget build(BuildContext context) {
  ...
  return Scaffold(
    appBar: AppBar(title: Text("${lblDiagnosis}")),
    body: SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          if (_image != null)
            Image.file(_image!, height: 200,width:1000)
          else if (_imageUrl.isNotEmpty)
            Image.network(_imageUrl, height: 200,width:1000)
          else
            const Icon(Icons.image_not_supported, size: 100),

          const SizedBox(height: 16),
          Text(_diagnosis ?? "No diagnosis", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ...
          if ((_chemicalProducts?.isNotEmpty ?? false)) ...[
            Text("${lblChemical} :", style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold)),
            ..._chemicalProducts!.map((e) => Text("• ${e['name']} by ${e['company']} (${e['content']}, dosage: ${e['dosage']}, cost: ${e['approx_cost']})"))
          ],

          const SizedBox(height: 12),
          if ((_organicTreatments?.isNotEmpty ?? false)) ...[
            Text("${lblOrganic} :", style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold)),
            ..._organicTreatments!.map((e) => Text("• $e")),
          ],
        ],
      ),
    ),
