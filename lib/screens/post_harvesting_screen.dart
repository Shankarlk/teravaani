...
class PostHarvestingScreen extends StatefulWidget {
  ...
  const PostHarvestingScreen({Key? key, required this.langCode}) : super(key: key);
...
Future<void> _checkAndShowReminders() async {
  ...
  for (final event in events) {
    ...
    if (diff == 2 || diff == 1 || diff == 0) {
      ...
      final messageNative = "🧾 $eventLabelNative: $eventTypeNative\n📅 $dateLabelNative: $formattedDate";
...
Future<List<Map<String, dynamic>>> fetchUpcomingEvents(String userId) async {
  final url = Uri.parse("https://teravaanii-hggpe8btfsbedfdx.canadacentral-01.azurewebsites.net/api/upcoming-events/$userId");
    ...
    _speech.listen(
      ...
      onResult: (result) async {
        if (result.finalResult) {
          ...
          String translatedToEnglish = await translateNativeToEnglish(spokenText);
          print("👂 User said (native): $spokenText ➜ (english): $translatedToEnglish");

          if (waitingForCrop) {
            if (translatedToEnglish.trim().toLowerCase() != "tomato") {
              final error = await translateToNative("Only 'tomato' crop is supported currently");
              setState(() {
                errorMessage = error;
            ...
            final currentPromptnt = await translateToNative("Please say the sowing date");
            ...
            try {
              ...
              final error = await translateToNative("Invalid date. Please say a valid date like 25 July 2025.");
              setState(() {
                errorMessage = error;
                hasValidationError = true;
              });
...
Future<String> getOrCreateDeviceId() async {
  ...
  Future<void> sendDataToApi() async {
    try {
      ...
      final url = Uri.parse("https://teravaanii-hggpe8btfsbedfdx.canadacentral-01.azurewebsites.net/api/generate-calendar");
      final response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
        body: json.encode({
      ...
      if (response.statusCode == 200) {
        ...
        String responseMessageNt = await translateToNative(responseData["message"]);
      ...
      print("Error: $e");
        String responseMessageNt = await translateNativeToEnglish("Failed To Save the Events");
      setState(() => responseMessage = responseMessageNt);
      await flutterTts.speak("❌ ${responseMessageNt}");
    }
  }
